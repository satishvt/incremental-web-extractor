    ...
    result_block = soupx.findAll("script", {"type": "application/ld+json"})
        ...
        for ptext in result_block:
            ...
            if statusc:
                ...
                try:
                    sendresult = producer.send('world', value=xyd_json)
                    try:
...
r = requests.get('https://www.firstpost.com/category/world')
...
for result in result_block:
    try:
        if (result['href'].startswith("https://www.firstpost.com/world/")) & (result['href'].endswith('.html')):
